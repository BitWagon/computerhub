...
export default function CategorySubcategories({ items }) {
  return (
    <section className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 sm:p-6">
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Explore products based on what you are looking for.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.slug}
              href={`/products?category=${item.parent}&subcategory=${item.slug}`}
